...
async def test_classify_regulation_search_procurement():
    result = await classify_query(
        "How does the procurement process work for small businesses?"
    )
    assert result.intent == QueryIntent.REGULATION_SEARCH
    assert result.confidence == 0.6
...
async def test_confidence_keyword_match_is_0_6():
    assert (
        await classify_query("What are the compliance requirements?")
    ).confidence == 0.6
...
def test_get_document_request_type_checklist():
    ...
    assert (
        get_document_request_type(
            "Generate a QC inspection checklist for demolition activities"
        )
        == "checklist"
    )
...
def test_get_document_request_type_form():
    ...
    assert get_document_request_type("Create a government property tracking form") in (
        "form",
        "letter",
    )
...
def test_get_document_request_type_letter():
    ...
    assert (
        get_document_request_type("Write a serial letter notifying the KO") == "letter"
    )
...
def test_is_document_request_only_letter():
    ...
    assert (
        is_document_request_query("Generate a QC inspection checklist for demolition")
        is False
    )
