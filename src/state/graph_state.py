...
class GovGigState(MessagesState, total=False):
    ...
    ui_action: Optional[
        Dict[str, Any]
    ]  # e.g. {"type": "navigate", "route": "/dashboard"}
    ...
    is_document_request: Optional[
        bool
    ]  # True only for letter-type; checklist/form use synthesis (from classifier)
    document_request_type: Optional[str]  # "letter" | "checklist" | "form" | None
    is_comparison: Optional[
        bool
    ]  # "REA vs change order", "type 1 vs type 2" (from classifier)
    is_construction_lifecycle: Optional[
        bool
    ]  # commissioning, punchlist, closeout (from classifier)
    ...
    quality_gate_healing: Optional[
        bool
    ]  # True only on the step that triggered healing; False on no-op passes
