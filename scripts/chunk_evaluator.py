...
def main():
    ...
    parser.add_argument("--sample-size", type=int, default=20, help="Number of chunks to sample for semantic review")
    args = parser.parse_args()

    cfg = _load_db_config()
    ns_like = f"{args.ns}%"
    enc = tiktoken.get_encoding("cl100k_base")
    anchor_re = re.compile(r"^\s*(?:FAR|DFARS)?\s*(?:52\.\d{3}-\d+|252\.\d{3}-\d+|\d{2,3}\.\d{3}(?:-\d+)?)\b", re.IGNORECASE)
    ...
    try:
        with psycopg.connect(conninfo) as conn:
            with conn.cursor() as cur:
                # 1. Basic Counts
                cur.execute("SELECT COUNT(*) FROM embeddings_dense WHERE namespace LIKE %s", (ns_like,))
                ...
                # 3. Token Analysis
                cur.execute("SELECT text, metadata FROM embeddings_dense WHERE namespace LIKE %s", (ns_like,))
                rows = cur.fetchall()
                ...
                print(f"Tiny Non-Anchor Chunks: {tiny_non_anchor} ({metrics['tiny_non_anchor_ratio']*100:.2f}%)")
                print(f"Duplicate Groups: {dup_groups} ({metrics['duplicate_ratio']*100:.2f}%)")
                print(f"Oversized Chunks (>800): {oversized} ({metrics['oversized_ratio']*100:.2f}%)")
                print("-" * 30)
